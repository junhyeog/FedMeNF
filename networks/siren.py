...
class Siren(nn.Module):
    def __init__(self, dim_in, dim_out, w0=1.0, c=6.0, is_first=False, use_bias=True, activation=None, dropout=0.0):
        super().__init__()
        self.dim_in = dim_in
...
class SirenNet(nn.Module):
    def __init__(
        ...
        self.last_layer = Siren(
            dim_in=dim_hidden, dim_out=dim_out, w0=w0, use_bias=use_bias, activation=final_activation
        )
    ...
    def forward(self, x, mods=None):
        ...
        # return self.last_layer(x) # original
        return self.last_layer(x) + 0.5  # to make the output in the range of [0, 1]
