...
class CusteomLEAF(LEAF):
    ...
    def __init__(
        ...
        if len(files):
            ...
            for file in files:
                ...
                dict_clients_train[file_idx] = torch.tensor(
                    range(len(train_data_total), len(train_data_total) + len(train_data)),
                    dtype=torch.int64,
                )
                ...
                dict_clients_test[file_idx] = torch.tensor(
                    range(len(test_data_total), len(test_data_total) + len(test_data)),
                    dtype=torch.int64,
                )
                ...
                if osp.exists(osp.join(self.processed_dir, file, "val.pt")):
                    ...
                    dict_clients_val[file_idx] = torch.tensor(
                        range(len(val_data_total), len(val_data_total) + len(val_data)),
                        dtype=torch.int64,
                    )
        ...
        self.dataset_train = CustomTensorDataset(
            name,
            train_data_total,
            train_targets_total,
            transform_train,
        ...
        self.dataset_test = CustomTensorDataset(
            name, test_data_total, test_targets_total, transform_val, target_transform
        )
        self.dataset_val = CustomTensorDataset(
            name, val_data_total, val_targets_total, transform_val, target_transform
        )
        self.dict_clients_train = dict_clients_train
        self.dict_clients_test = dict_clients_test
    ...
    def processed_dir(self):
        return osp.join(
            self.root,
            self.name,
            f"processed-w-{self.s_frac}-{self.tr_frac}-{self.val_frac}-{self.seed}",
        )
    ...
    # TODO: fix self.transform to self.transform_train and self.transform_val
    # def __getitem__(self, index):
    #     """
    #     Arguments:
    #         index (int): Index
    ...
    #             img = np.resize(imgs[idx].numpy().astype(np.uint8), IMAGE_SIZE[self.name])
    #             img = Image.fromarray(img, mode=MODE[self.name])
    #             if self.transform is not None:
    #                 img = self.transform(img)
        ...
        for num, file in enumerate(tqdm(files)):
            ...
            for writer, v in raw_data["user_data"].items():
                ...
                if self.name == "celeba":
                    ...
                    data = [
                        F.resize(
                            F.to_pil_image(data[i].view(original_size).permute(2, 0, 1)),
                            (32, 32),
                        )
                    ...
                    data = torch.stack(
                        [
                            (F.to_tensor(data[i]) * 255).long().permute(1, 2, 0).flatten()
                            for i in range(len(data))
                        ]
                    )

                train_data, test_data, train_targets, test_targets = train_test_split(
                    data, targets, train_size=self.tr_frac, random_state=self.seed
                )
